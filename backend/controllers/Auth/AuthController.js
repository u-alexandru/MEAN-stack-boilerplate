...
exports.login = async (req, res) => {
  try {
    ...
    if (foundUser && (await bcrypt_compare(password, foundUser.password))) {
      ...
      //Set auth cookie
      res.cookie("access_token", token, { maxAge: process.env.TOKEN_EXPIRATION * 1000, httpOnly: true, sameSite: 'none', secure: true });
...
exports.resetConfirmEmailToken = async (req, res) => {
  const token = req.body.token;
  //Initialize transaction
  const t = await sequelize.transaction();
  //Email validation logic starts here
...
exports.sentAccountValidationEmail = async (req, res) => {
...
exports.logout = async (req, res) => {
  ...
  try {
    ...
    // get token exp date
    console.log('here');
    await redisClient.set(`bl_${token}`, token);
    // get token expiration date in seconds
    const tokenExp = decodedToken.exp;
    const now = Math.floor(Date.now() / 1000);
    const tokenExpInSec = tokenExp - now;
    // set token in redis
    await redisClient.expire(`bl_${token}`, tokenExpInSec);
    console.log('redisClient', await redisClient.get(`bl_${token}`));
    res.status(201).json({
      success: true,
      message: 'Logged out.',
    });
